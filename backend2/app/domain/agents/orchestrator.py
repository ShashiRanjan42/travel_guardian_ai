...
class Orchestrator:
    ...
    async def process_event(self, event_id: str):
        ...
        async with AsyncSessionLocal() as session:
            ...
            # Find Traveller for the affected booking
            matched_leg_id = correlator_res["direct_match_ids"][0]
            from app.data.models import Booking, Itinerary, ItineraryVersion, Traveller
            stmt = select(Booking.id, Itinerary.id, Traveller).join(Itinerary, Booking.id == Itinerary.booking_id).join(ItineraryVersion, Itinerary.id == ItineraryVersion.itinerary_id).join(Leg, ItineraryVersion.id == Leg.version_id).join(Traveller, Booking.traveller_id == Traveller.id).where(Leg.id == matched_leg_id)
            booking_result = await session.execute(stmt)
            row = booking_result.first()
            if not row:
                return
            booking_id, itinerary_id, traveller = row
            ...
            ctx = AgentContext(
                ...
                itinerary=None,
                traveller_profile={
                    "is_solo": getattr(traveller, 'is_solo', True),
                    "vulnerability_flag": getattr(traveller, 'vulnerability_flag', False),
                    "preferences": getattr(traveller, 'preferences', {})
                },
                prior_results={}
            )
